/**
 * EvilEye by React Bits (https://reactbits.dev/backgrounds/evil-eye)
 ...
 * - Desktop-wide gaze: the container mousemove/mouseleave listeners are
 *   replaced by targetX/targetY props (normalised -1..1) fed from the main
 *   process global cursor position. The original smoothing is kept.
 * - Uniforms update without re-creating the WebGL context when visual props
 *   change, so state-driven glow changes do not flicker.
 * - Device pixel ratio aware rendering for crisp output at small sizes.
 */
...
interface EvilEyeProps {
  ...
  pupilFollow?: number
  flameSpeed?: number
  /** Normalised gaze target, -1..1, positive x is right. */
  targetX?: number
  /** Normalised gaze target, -1..1, positive y is up. */
  targetY?: number
}
...
void main() {
  ...
  // Transparency adaptation: alpha follows brightness so the flame glow fades
  // into the desktop, while a soft dark core keeps the eye body opaque enough
  // that the pupil reads as dark rather than see-through.
  float alpha = max(color.r, max(color.g, color.b));
  float core = (1.0 - smoothstep(0.30, 0.55, length(uv))) * 0.92;
  alpha = clamp(max(alpha, core), 0.0, 1.0);

  // Premultiplied output: colour is already <= alpha per channel.
...
export default function EvilEye({
  ...
  // One-time WebGL setup. Visual props are synced through uniforms below so
  // state changes never tear down and rebuild the context.
  useEffect(() => {
    ...
    program = new Program(gl, {
      ...
      uniforms: {
        ...
        uPupilSize: { value: p.pupilSize },
        uIrisWidth: { value: p.irisWidth },
        uGlowIntensity: { value: p.glowIntensity },
        uIntensity: { value: p.intensity },
        uScale: { value: p.scale },
        uNoiseScale: { value: p.noiseScale },
        uMouse: { value: [0, 0] },
        uPupilFollow: { value: p.pupilFollow },
        uFlameSpeed: { value: p.flameSpeed },
        uEyeColor: { value: hexToVec3(p.eyeColor) }
      }
    })
    ...
    function update(time: number): void {
      ...
      mouse.x += (targetRef.current.x - mouse.x) * 0.05
      mouse.y += (targetRef.current.y - mouse.y) * 0.05
  ...
  // Sync visual props into uniforms without re-initialising WebGL.
  useEffect(() => {
    const program = programRef.current
    if (!program) return
    program.uniforms.uPupilSize.value = pupilSize
    program.uniforms.uIrisWidth.value = irisWidth
    ...
    program.uniforms.uPupilFollow.value = pupilFollow
    program.uniforms.uFlameSpeed.value = flameSpeed
    program.uniforms.uEyeColor.value = hexToVec3(eyeColor)
  }, [
    eyeColor,
    intensity,
    pupilSize,
