...
async def test_headline_publication_range_is_half_open_and_undated_items_are_not_backfilled(days):
    ...
    rows = (
        news("boundary", title="At start", published_at=boundary),
        news("older", title="Before start", published_at=boundary - timedelta(microseconds=1)),
        news("end", title="Before end", published_at=NEWS_NOW - timedelta(microseconds=1)),
        news("at-end", title="At end", published_at=NEWS_NOW),
        news("future", title="After end", published_at=NEWS_NOW + timedelta(seconds=1)),
        news("undated", title="No publication date", published_at=None),
    )
...
async def test_wider_window_adds_retained_older_news_and_disabled_sources_stay_empty():
    rows = tuple(
        news(
            str(day), title=f"Published {day} days ago", published_at=NEWS_NOW - timedelta(days=day)
        )
        for day in (1, 4, 6, 13)
    )
    selected, _ = service(rows)
