"""Team relationship review follows current membership, authorship and archive policy."""
...
async def test_member_creates_on_shared_report_then_revocation_hides_history(
    ...
    async with container.session_factory() as session:
        root, _ = await container.report_relationships(session).get(claims, first.relationship_id)
    ...
    async with container.session_factory() as session:
        ...
        with pytest.raises(NotFound):
            await service.get(claims, first.relationship_id, first.id)
        with pytest.raises(NotFound):
...
async def test_other_member_reads_but_cannot_correct_existing_review(
    ...
    async with container.session_factory() as session:
        service = container.report_relationships(session)
        assert (await service.get(user_claims, first.relationship_id))[1] == first
        with pytest.raises(Forbidden):
            await service.update(user_claims, first.relationship_id, first.id, VALUE, CONTEXT)
...
async def test_archive_allows_read_and_admin_override_only(client, container, admin, user):
    ...
    async with container.session_factory() as session:
        ...
        with pytest.raises(Forbidden):
            await service.create(user_claims, record.id, 1, VALUE, CONTEXT)
        second = await service.update(admin_claims, first.relationship_id, first.id, VALUE, CONTEXT)
...
async def test_team_quota_is_shared_across_authors(client, container, admin, user, monkeypatch):
    ...
    monkeypatch.setattr(relationships, "MAX_SCOPE_DECISIONS", 1)
