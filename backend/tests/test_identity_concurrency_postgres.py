...
from test_report_identity_teams import team_report
...
async def settings(settings):
    source = os.environ.get("ASE_IDENTITY_CONCURRENCY_POSTGRES_URL")
    if not source:
        pytest.skip("Set ASE_IDENTITY_CONCURRENCY_POSTGRES_URL to an owned disposable PG server")
    url = make_url(source)
    assert url.drivername == "postgresql+asyncpg"
    assert url.host in {"127.0.0.1", "localhost", "::1"}
    name = f"ase_identity_race_{uuid4().hex}"
...
def simultaneous_lock(monkeypatch):
    ...
    async def lock(repository):
        if len(pids) < 2:
            pids.append(await repository._session.scalar(text("SELECT pg_backend_pid()")))
            if len(pids) == 2:
                ready.set()
...
async def retained(container):
    async with container.session_factory() as session:
        roots = list(await session.scalars(select(IdentityDecisionRow)))
        revisions = list(await session.scalars(select(IdentityRevisionRow)))
        audited = await session.scalar(
            select(func.count())
            .select_from(AuditLogRow)
            .where(AuditLogRow.action.in_(["identity.created", "identity.revised"]))
        )
...
async def test_last_identity_quota_slot_serialises_independent_writers(
    ...
    async with container.session_factory() as session:
        repository = container.report_identities(session).identities
        initial_count, initial_bytes = await repository.scope_usage(
            user.id, team.id if team else None
        )
    assert initial_count == 1
    if quota == "count":
        monkeypatch.setattr(identities, "MAX_SCOPE_DECISIONS", 2)
    else:
        # Same frozen candidate/rationale and fixed-width IDs give equal charged sizes.
        # The allowance comes from actual persisted usage, not a stubbed quota repository.
        monkeypatch.setattr(identities, "MAX_SCOPE_BYTES", 2 * initial_bytes)
    pids = simultaneous_lock(monkeypatch)
    ...
    async with container.session_factory() as session:
        ...
        assert (await service.get(user_claims, initial.decision_id, initial.id))[1] == initial
...
async def test_same_base_service_corrections_keep_one_winner_and_immutable_history(
    ...
    async def correct(rationale):
        async with container.session_factory() as session:
            return await container.report_identities(session).update(
                claims, first.decision_id, first.id, replace(VALUE, rationale=rationale), CONTEXT
            )
    ...
    async with container.session_factory() as session:
        ...
        assert (await service.get(claims, first.decision_id))[1] == winner
...
async def test_repository_compare_and_swap_is_atomic_without_application_guard(
    ...
    async def append(proposal):
        async with container.session_factory() as session:
            repository = container.report_identities(session).identities
            assert (await repository.get(first.decision_id)).latest_revision_id == first.id
...
async def test_membership_revocation_while_writer_waits_denies_correction(
    ...
    async def lock(repository):
        ...
        if name in {"identity-revoker", "identity-writer"}:
            pids[name] = await repository._session.scalar(text("SELECT pg_backend_pid()"))
        if name == "identity-writer":
            waiting.set()
        await original(repository)
        if name == "identity-revoker":
            locked.set()
    ...
    async def correct():
        async with container.session_factory() as session:
            return await container.report_identities(session).update(
                claims, first.decision_id, first.id, VALUE, CONTEXT
            )

    revoker = asyncio.create_task(revoke(), name="identity-revoker")
    writer = None
    try:
        await asyncio.wait_for(locked.wait(), 10)
        writer = asyncio.create_task(correct(), name="identity-writer")
