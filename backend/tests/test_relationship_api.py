"""Relationship HTTP contracts, immutable history and server-owned anchors."""
...
from test_report_relationship_service import seed_report
...
async def test_create_history_correction_and_version_listing(client, container, user):
    ...
    payload = body(record)
    response = await client.post("/api/relationship-reviews", headers=headers, json=payload)
    assert response.status_code == 201, response.text
    assert response.headers["cache-control"] == "no-store"
    first = response.json()
    assert first["assertion"]["evidence_label"] == "E1" and "subject" not in first
    assert first["report_version_id"] == str(version.id)
    assert first["authored_by"] == str(user.id)
    path = f"/api/relationship-reviews/{first['relationship_id']}"
    patch = {
        key: value for key, value in payload.items() if key not in {"report_id", "version_number"}
    }
    patch.update(base_revision_id=first["id"], disposition="disputed")
    ...
    assert (
        await client.post("/api/relationship-reviews", headers=headers, json=payload)
    ).status_code == 409
...
async def test_boundary_rejects_client_owned_anchors_and_invalid_values(
    ...
    response = await client.post(
        "/api/relationship-reviews", headers=headers, json={**body(record), field: value}
    )
...
async def test_authentication_and_page_bounds(client, container, user):
    record, _ = await seed_report(container, user)
    assert (await client.post("/api/relationship-reviews", json=body(record))).status_code == 401
    headers = bearer(await login_token(client, user.email, USER_PASSWORD))
