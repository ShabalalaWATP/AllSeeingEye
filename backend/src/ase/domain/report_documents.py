...
MAX_DIAGRAM_SVG_CHARS = 60_000


@dataclass(frozen=True, slots=True)
class DocumentDiagram:
    """An application-drawn vector diagram and the text that must accompany it.

    The markup is generated by the application from validated structured data, never
    taken from a model or a source. ``alt_text`` and the equivalent table that the
    document places beside it are mandatory, so the diagram is never the only way to
    read the content.
    """
    ...
    def __post_init__(self) -> None:
        if (
            not self.title
            or not self.alt_text
            or not self.svg.startswith("<svg ")
            or not self.svg.endswith("</svg>")
            or len(self.svg) > MAX_DIAGRAM_SVG_CHARS
            or any(ord(character) < 32 and character not in "\n\t" for character in self.svg)
            or len(self.citation_numbers) > 32
            or any(type(number) is not int or number < 1 for number in self.citation_numbers)
            or len(set(self.citation_numbers)) != len(self.citation_numbers)
        ):
            raise ValueError("Invalid bounded document diagram")
...
class DocumentBlock:
    ...
    diagram: DocumentDiagram | None = None
    ...
    def __post_init__(self) -> None:
        ...
        payloads = (
            ...
            + int(self.figure is not None)
            + int(self.diagram is not None)
        )
        if payloads > 1:
        ...
        if self.kind is BlockKind.DIAGRAM and self.diagram is None:
            raise ValueError("A diagram block requires a diagram")
...
class ReportDocument:
    ...
    def __post_init__(self) -> None:
        ...
        cited.update(
            ...
            for number in block.diagram.citation_numbers
        )
        if not cited <= known:
