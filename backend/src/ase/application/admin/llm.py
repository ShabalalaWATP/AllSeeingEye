...
class TestLlmProfileUseCase:
    """Sends a one-line prompt that must come back as `{"ok": true}`; records the usage."""
    ...
    async def execute(self, actor: User, profile_id: UUID, context: RequestContext) -> TestOutcome:
        ...
        outcome = await self._call(profile, request)
    ...
    async def _call(self, profile: LlmProfile, request: LlmRequest) -> TestOutcome:
        ...
        return TestOutcome(ok=True, latency_ms=result.latency_ms, model=result.model)
