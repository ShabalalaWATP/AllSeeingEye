...
class Producer:
    ...
    async def produce_with_claims(  # noqa: PLR0915 - serial checkpointed report stages
        ...
        *,
        ...
        if snapshot is None:
            direction = await direct_for_job(job, profile_for, totals, self._gateway, self._cipher)
        ...
        if (
            ...
            body, advocacy = await advocate_for_job(
                job, profile_for, body, selection.items, totals, self._gateway, self._cipher
            )
