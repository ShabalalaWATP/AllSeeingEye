...
See [Map workspace](MAP_WORKSPACE.md) for saved objects and studies, and
[Area research](AREA_RESEARCH.md) for boundary collection and monitoring.
...
GNSS, BNG and day/night shading also start off. Basemap and lite-mode preferences
remain saved. Operators can enable other layers for the current session.

Conflict controls include a searchable regional overview alongside individual
...
**Radio and terrain**, and **Map settings**. Search by name or description and pin
frequently used tools. Zoom, north, world view and fullscreen remain separate
navigation controls. The Map guide and global search open the same tools.

The desktop inspector resizes between 320 and 600 pixels, including by keyboard.
On narrow screens it becomes a collapsible bottom sheet. Collapse exposes more
map space while retaining the tool; Close ends the open inspector. Escape and
Close return focus to the opening control. Panels remain non-modal so map
gestures are available. Reduced-motion preferences disable brief transitions.

**On this map** lists the current sketch, measurement, research boundary, radio
result, route and terrain study, plus collection drawings. Reopen a tool, hide
its result or clear it. Hiding does not delete inputs or save a document. Save
drawing collections and radio studies explicitly in their respective tools.
...
- Measurement offers labelled Distance/Area choices, a prominent result and
  ...
  notes are expandable. Keyboard shortcuts are described below.
...
CCTV, infrastructure, satellite and context groups share the inspector. Tool
discovery and layout are separate from map input ownership. Geometry, history,
research handoffs, persistence and propagation calculations have separate
responsibilities. Repositories enforce access and revision checks. These
boundaries apply SOLID without introducing a general plugin framework.

## Reference catalogue provenance
...
Draw paths, polygons, two-corner rectangles or centre/radius circles. Temporary
drafts reset on account/workspace access changes. Add finished sketches to a
collection and save it to keep it between visits. Collections support up to
50 named objects, including coordinate-entered points, with notes, colour,
visibility, locking, coordinate editing, duplication and removal. Sketch and
collection undo/redo are separate. GeoJSON import/export uses bounded validation.
Sketch geometry is bounded to 32 anchors and a 1,000 km circle radius.
...
coalesced to animation frames. The current sketch is separate from collection
objects. Moving a circle preserves its radius; other shapes retain
...
Supported collection areas can initialise research, and selected points can
initialise radio sites. Geodesic sketch strokes and straight-edged canonical
research boundaries have different rendering semantics. Area research and
exact-shape watches use the canonical boundary, not its enclosing rectangle.
Dateline rectangles are split; unsplit crossing polygons require a supported
boundary. This is a lightweight planning editor, not a full GIS topology editor.
...
The RF calculator separates three propagation choices from the free-space
...
Sites support precise coordinate entry, labels and reuse of suitable drawing or
measurement positions. Named studies retain inputs and bounded results, preserve
a baseline for comparison, and export assumptions and provenance. Profile
interaction highlights a sampled location on the map. Reopening a study does
not prove that its data is current; review provenance and recalculate when needed.
...
shaded 360Ã‚Â° reference uses that same limit; it does not establish reception.
...
shows a terrain profile. The selectable 360Ã‚Â° area study screens 24 bearings
...
The grouped catalogue has 25 choices, including four Bowman scenarios,
AN/PRC-150(C), PRC-152A, PRC-117G, Thales PRC-148, SINCGARS RT-1702 and the
...
Media is selection-driven. Source coverage and provider availability are not
established by styling or a catalogue entry.
...
deadline. The fixed HTTPS provider uses the protected transport, without
...
This setting belongs in the operator's environment configuration.
...
`ASE_OS_MAPS_KEY`. The style panel
groups these choices with a visible readiness state, setup instructions and
...
## Area research and watches

Choose **Research** for a supported collection shape, or open **Research area**
and draw a boundary. **What is already loaded here?** separates precise inside
observations from approximate markers, outside, country-level and unlocated
records. This local preview uses publication dates and the map's loaded sample.
It makes no provider or AI call. **Check sources** checks collection capabilities
and budgets; it does not establish that external evidence was found.

The question, period, depth and selected sources survive tool changes. **Continue
in full research** carries the private draft to the research form. Choose Basic,
Deep or Advanced research, review disclosure and generate a personal report.
Submitted report jobs are durable: closing an inspector does not cancel an
admitted server job. The report retains exact geometry, interval and evidence
receipts. See [Area research](AREA_RESEARCH.md) for collection limits.

**Save as a reusable area** stores exact canonical geometry and its hash in Plans
& areas. **Research area** on a saved record reuses that boundary. Exact-area
collection plans support evidence matching; their report action opens standalone
personal area research and explains that plan requirements are not transferred.
...
outside the shape. Neither workflow detects arrivals or boundary crossings.

## Terrain and coordinates

**Terrain profile and visibility** reuses the bounded elevation service. Enter
sites directly or reuse suitable drawing points. A profile samples elevation
along a path. The sampled viewshed screens geometric visibility from an observer
height in selected directions. It is not a dense visibility raster, a radio model
or a building-aware survey. Missing elevations remain unknown; provenance and
sampling limitations accompany the result.

The coordinate workbench accepts WGS84 decimal degrees, degrees/minutes/seconds
and supported UTM input. It displays conversions, centres the map and copies a
latitude/longitude pair. British National Grid remains a separate overlay.
Conversion does not improve the precision of a source position.

## Nuclear education

The nuclear education panel links to NUKEMAP and its explanation of modelling
limits. It opens an external educational resource in another tab. The link
contains no selected locations, drawings or research questions. Opening the
panel itself does not load the external site.

This is an attributed reference, not an integrated simulator or an observed
hazard layer. Its illustrations are approximate and are not emergency planning
or response guidance. External terms and availability apply.

## Validation limits

Automated checks cover geometry validation, input ownership, private state
invalidation, bounded provider work, document revisions and supported handoffs.
Numerical tests do not establish measured radio reception, survey-grade terrain
accuracy, exhaustive live-source coverage or graphics-driver compatibility.
Release checks are recorded in the implementation plan and development story.
