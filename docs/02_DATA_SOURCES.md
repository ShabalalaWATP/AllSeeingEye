...
| Reuters and AP | Original research found no official RSS | | | | | | B when the source is established | Candidate coverage through aggregators | Reuters also has an implemented outlet YouTube channel, read through the keyed Data API v3 route. Google News watchlist items currently remain aggregator C; URL decoding does not automatically assign an outlet grade |
...
| Reddit | Subreddit Atom listings | None | Not collected | Not collected | No | `robots.txt` is `User-agent: *` then `Disallow: /`, checked 16 September 2026 | n/a | 5, retired 16 September 2026 | The three subreddit feeds and their research providers were removed. Reddit's Public Content Policy directs programmatic access to the Data API with a registered OAuth application; that is not built (see [SOCIAL_SOURCE_COVERAGE.md](SOCIAL_SOURCE_COVERAGE.md)) |
| YouTube Data API v3 | Reviewed channel uploads: title, publication time, channel identity, link and a bounded description excerpt | `ASE_YOUTUBE_API_KEY`, server-side | 10,000 quota units a day by default; 1 unit per `channels.list` resolution and per `playlistItems.list` poll, about 1,392 units a day for 29 channels every 30 minutes | Every 30 minutes per channel, paced 1 s apart on `www.googleapis.com` | No coordinates supplied by this connector | YouTube Data API v3 terms; no transcripts, captions, comments or media | Outlet grade for outlet channels, B for official bodies, E with credibility 6 for analysis channels, `state_controlled` for CGTN | 5, rebuilt 16 September 2026 | Replaces the channel Atom feeds, which `robots.txt` disallows at `/feeds/videos.xml`. No key means no YouTube collection at all. Not verified against the live API: no key exists on this machine |
...
| Reddit | `r/worldnews/new/.rss` | 200 with the project's User-Agent, but `robots.txt` is `Disallow: /` for every user agent | Retired 16 September 2026; the compliant route is a registered OAuth application under the Public Content Policy, not built |
| YouTube | channel RSS | 200, but `robots.txt` disallows `/feeds/videos.xml` | Channel feeds retired 16 September 2026 and replaced by the keyed Data API v3 route |
...
The social board at `/trackers/social` reads the bounded live store. It shows the retained day's posts by platform and instance, top hashtags, the latest 50 posts and located-post counts. The existing social globe category displays events with a location; the Mastodon and YouTube connectors do not invent coordinates. Public Mastodon posts start at E6, while outlet YouTube channels retain their outlet reliability and independent analysis channels sit at E6. Reddit collection was retired on 16 September 2026 because `robots.txt` disallows every path. Neither burst activity nor translation raises a source's grade.
