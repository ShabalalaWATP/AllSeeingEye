...
Status: proposal. Facts below were checked against official documentation on 2 September 2026 by research agents; anything they could not confirm is marked UNVERIFIED. This file becomes `SOURCES.md` in the repo and seeds the source registry.
...
| Google News RSS | Top stories, topic, geo and keyword-search feeds per edition (`hl`, `gl`, `ceid`), with `when:1d` style queries | None | About 100 items per feed; undocumented; heavy IPs may be blocked | Continuous | Edition only | Google terms; undocumented | Inherits outlet, else C | 1 | Foreign-language headline coverage for every country. Article links are encoded and need decoding through Google's `batchexecute` endpoint, so decode lazily and only for cited evidence |
...
| Reuters and AP | No official RSS (Reuters retired it in 2020; AP has none) | | | | | | B | 1 | Reached through Google News and GDELT; graded B once the outlet is identified from the decoded URL |
...
| Source | What | Auth | Limits | Cadence | Geo | Licence note | Reliability | Phase | Notes |
|---|---|---|---|---|---|---|---|---|---|
| Bluesky public AppView | `searchPosts`, author feeds, custom feeds | None | 3,000 requests per 5 min per IP | Real time | No | Bluesky terms; users own content | E (named OSINT accounts may be raised) | 5 | Keyword and hashtag watchlists; bulk history pulls are refused, so poll recent |
...
| Mastodon | Public and hashtag timelines per instance | None unless the instance disables public preview | 300 requests per 5 min per IP; 40 per page | Real time | No | Per-instance rules | E | 5 | Instances chosen by the admin |
| Reddit | Subreddit listings and search | Free OAuth app | 100 requests/min per client; 10/min without OAuth | Real time | No | Free for non-commercial; descriptive User-Agent | E | 5 | Subreddits such as r/CombatFootage and r/worldnews as watchlists |
| YouTube channel RSS | 15 latest videos per channel | None | None | Real time | No | YouTube terms | E (channel-specific override) | 5 | `youtube.com/feeds/videos.xml?channel_id=...` verified; the Data API's 10,000 units/day is unnecessary |
| Telegram | Public channels via MTProto (Telethon, your own account), Bot API (only channels the bot administers), or the public `t.me/s/channel` preview pages | Varies | Varies | Real time | No | API terms ban AI training on Telegram data; MTProto with fresh accounts risks bans; preview scraping is unofficial | E | Open question 7 | Not in the first release |
