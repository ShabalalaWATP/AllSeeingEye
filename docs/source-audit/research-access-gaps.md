...
| Social | Configured Mastodon watchlists, plus 29 reviewed YouTube channels through the keyed Data API v3, in live collection; one aggregated `research-youtube` video search for question research (retired 16 September 2026: the Reddit and YouTube feed routes their hosts' `robots.txt` disallow, and their nine `research_social_*` providers) | No replies or video analysis, and no Reddit collection at all. Video search needs a key and is capped at 40 searches a day. Posts usually have no reliable geography. A channel is not an independent origin from its publisher |
...
| ACLED events | `ASE_ACLED_REFRESH_TOKEN` (or `ASE_ACLED_ACCESS_TOKEN`) plus event-data entitlement | The app renews access from an operator-supplied refresh token (updated 15 September 2026); a manual access token is not renewed. A personal Gmail address receives Open myACLED aggregated access. That does not unlock the current event adapter. Do not claim institutional affiliation or promise a free event-level feed. [Access tiers](https://acleddata.com/myacled-faqs) |
