...
import type { RelationshipUpdate } from '@/lib/api/relationships';
...
it('allows explicit conflict removal, replacement and withdrawal without rewriting previous history', async () => {
  let posted: RelationshipUpdate | null = null;
  const saved = vi.fn();
  const current = {
  ...
  server.use(
    http.patch('/api/relationship-reviews/relationship-1', async ({ request }) => {
      posted = (await request.json()) as RelationshipUpdate;
  ...
  await userEvent.selectOptions(screen.getByLabelText('Relationship assessment'), 'withdrawn');
  await userEvent.type(
  ...
  expect(current.unresolved_conflicts).toEqual(['Current validity is unknown.']);
