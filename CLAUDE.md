...
- Authentication checks the current active user, security version and live refresh family on every protected request. JWTs require `sid` and `sv`; old-format access JWTs fail, while valid existing refresh cookies may rotate. All users can enrol email or authenticator MFA; administrators require an MFA-verified session and mandatory enrolment when no factor exists. See docs/MFA_OPERATIONS.md. Routes re-validate before release through `FenceDep` (`api/session_fence.py`), never by reading sessions directly; an architecture test enforces the fence or a reviewed allowlist. Release fences and streams may reuse a check for `ASE_SESSION_RECHECK_SECONDS` (default 15) unless a committed session or access change was signalled since (ADR 0021). Streams re-check at that interval, on any signal and before every alert, filter alerts by scope and emit `access.changed` for client invalidation.
...
- Backups are documented in `docs/BACKUP_RESTORE.md`. `backup.py` and `restore.py` never schedule or delete. `scheduled_backup.py`, run from the operator's crontab, encrypts and round-trips each archive, and prunes only its own `scheduled-*.tar.gpg` files (keep at least 3). Off-site copies and heartbeat alerts are opt-in. Restore drills target fresh destinations and preserve the encryption key separately unless `.env` inclusion is explicitly selected.
